...
const sponsors: Sponsor[] = [
  {
    ...
    type: "hotel",
    image: "/images/hotel-julio-cesar.jpg",
    description:
...
function SponsorCard({ sponsor }: { sponsor: Sponsor }) {
  return (
    <div className="bg-white rounded-sm overflow-hidden border border-[#0d1b3e]/10 flex flex-col group hover:shadow-xl hover:border-[#c8102e]/30 transition-all duration-300 hover:-translate-y-1">
      {/* Image */}
      <div className="relative aspect-video overflow-hidden">
        ...
        />
        {/* Badge */}
        <div className="absolute top-3 left-3 bg-[#c8102e] text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-sm">
          {sponsor.badge}
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        <h3
          className="text-[#0d1b3e] font-black text-xl uppercase leading-tight"
          style={{ fontFamily: "'Barlow Condensed', sans-serif" }}
        >
          {sponsor.name}
        </h3>
        <p className="text-[#0d1b3e]/65 text-sm leading-relaxed">{sponsor.description}</p>

        <div className="flex flex-col gap-2 mt-auto pt-3 border-t border-[#0d1b3e]/10">
          {sponsor.location && (
            <div className="flex items-start gap-2">
              <MapPin size={13} className="text-[#c8102e] shrink-0 mt-0.5" />
              <span className="text-[#0d1b3e]/60 text-xs">{sponsor.location}</span>
            </div>
          )}
          {sponsor.discount && (
            <div className="flex items-start gap-2">
              <Tag size={13} className="text-[#c8102e] shrink-0 mt-0.5" />
              <span className="text-[#c8102e] font-semibold text-xs">{sponsor.discount}</span>
            </div>
          )}
          {sponsor.instagram && (
            <div className="flex items-center gap-2">
              <Instagram size={13} className="text-[#c8102e] shrink-0" />
              <span className="text-[#0d1b3e]/60 text-xs">{sponsor.instagram}</span>
            </div>
          )}
        </div>
      </div>
    ...
    <section id="sponsors" className="py-20 md:py-28 bg-[#f5f6fa]">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="flex flex-col items-center text-center mb-14">
          <span className="text-[#c8102e] text-xs font-black uppercase tracking-widest mb-3">Colaboradores</span>
          <h2
        ...
        <div className="mt-12 text-center">
          <p className="text-[#0d1b3e]/50 text-sm">
            ...
            <a href="mailto:femipat@gmail.com" className="text-[#c8102e] font-semibold hover:underline">
              femipat@gmail.com
            </a>
          </p>
        </div>
